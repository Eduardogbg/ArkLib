...
theorem completeness [Nonempty σ] :
  ...
  exact ⟨by {
    ...
    · simp only [Option.getM, pure_bind] at hx
      ...
      · -- The two output oracle statements agree: both resolve (via `mkVerifierOStmtOut`) to the
        -- single forwarded oracle statement `oStmt default`.
        rw [OracleVerifier.mkVerifierOStmtOut_inl (j := default) (h := rfl),
          OracleVerifier.mkVerifierOStmtOut_inr (j := ⟨0, by simp⟩) (h := rfl)]
        rfl
      · funext i
        rcases i with j | j <;> { have hj : j = default := Unique.uniq _ j; subst hj; rfl }
  }⟩
