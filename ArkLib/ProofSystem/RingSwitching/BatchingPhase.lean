...
/-- High-priority alias so the concrete `pSpecBatching` challenge-oracle `Fintype` instance
wins typeclass resolution over a generic (dead-ending) instance introduced by the VCVio bump. -/
instance (priority := 10000) instFintypePSpecBatchingChallenge' :
    ([(pSpecBatching κ L K).Challenge]ₒ).Fintype :=
  Binius.RingSwitching.instFintypePSpecBatchingChallenge κ L K

/-- High-priority alias so the concrete `pSpecBatching` challenge-oracle `Inhabited` instance
wins typeclass resolution over a generic (dead-ending) instance introduced by the VCVio bump. -/
instance (priority := 10000) instInhabitedPSpecBatchingChallenge' :
    ([(pSpecBatching κ L K).Challenge]ₒ).Inhabited :=
  Binius.RingSwitching.instInhabitedPSpecBatchingChallenge κ L K

/-- High-priority `Fintype` instance for the combined oracle spec used by the batching reduction,
so `HasEvalSPMF`/`probEvent` typeclass searches short-circuit past the generic dead-ending
instance introduced by the VCVio bump. -/
instance (priority := 10000) instFintypeBatchingAppendChallenge :
    (([]ₒ : OracleSpec PEmpty) + [(pSpecBatching κ L K).Challenge]ₒ).Fintype :=
  inferInstance

/-- High-priority `Inhabited` instance for the combined oracle spec used by the batching
reduction; companion to `instFintypeBatchingAppendChallenge`. -/
instance (priority := 10000) instInhabitedBatchingAppendChallenge :
    (([]ₒ : OracleSpec PEmpty) + [(pSpecBatching κ L K).Challenge]ₒ).Inhabited :=
  inferInstance

/-- High-priority `HasEvalSPMF` instance for the batching reduction's monad, so `probEvent`
lemmas apply without the typeclass search hitting the generic dead-ending oracle-spec `Fintype`
instance introduced by the VCVio bump. -/
noncomputable instance (priority := 10000) instHasEvalSPMFBatching :
    HasEvalSPMF (OptionT (OracleComp
      (([]ₒ : OracleSpec PEmpty) + [(pSpecBatching κ L K).Challenge]ₒ))) :=
  inferInstance

/-- High-priority `HasEvalSPMF` instance for the batching reduction's bare `OracleComp` monad;
companion to `instHasEvalSPMFBatching`. -/
noncomputable instance (priority := 10000) instHasEvalSPMFBatchingOracleComp :
    HasEvalSPMF (OracleComp
      (([]ₒ : OracleSpec PEmpty) + [(pSpecBatching κ L K).Challenge]ₒ)) :=
  inferInstance

/-- High-priority `Fintype` instance for the batching reduction's combined oracle spec, so the
soundness-unrolling lemma's `[(oSpec + [pSpec.Challenge]ₒ).Fintype]` argument resolves without the
generic VCVio search dead-ending after the dependency bump. -/
instance (priority := 10000) instFintypeBatchingSumSpec :
    (([]ₒ : OracleSpec PEmpty) + [(pSpecBatching κ L K).Challenge]ₒ).Fintype :=
  inferInstance
...
def batchingProverWitOut (stmtIn : BatchingStmtIn L ℓ) (witIn : BatchingWitIn L K ℓ ℓ')
  ...
  let h_poly : ↥L⦃≤ 2⦄[X Fin ℓ'] :=
    Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ') (t := witIn.t')
...
def batchingStepLogic :
  ...
  proverOut := fun stmtIn witIn oStmtIn transcript =>
    ...
    let witOut := batchingProverWitOut (κ:=κ) (L:=L) (K:=K) β (ℓ:=ℓ) (ℓ':=ℓ') (h_l:=h_l)
...
def batchingKStateProp {m : Fin (2 + 1)}
  ...
  | ⟨2, _⟩ => by -- implied by relOut
    ...
    let witOut : SumcheckWitness L ℓ' 0 := {
      t' := witMid.t',
      H := Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ') (t := witMid.t')
        (m := (RingSwitching_SumcheckMultParam κ L K β ℓ ℓ' h_l).multpoly (ctx := ctx))
        (i := 0) (challenges := Fin.elim0)
    }
    exact
...
noncomputable def batchingKnowledgeStateFunction :
  ...
  toFun_full := fun ⟨stmtIn, oStmtIn⟩ tr witOut h_relOut => by
    ...
    · simp only [Fin.isValue, h_V_check, ↓reduceIte, OptionT.run_pure, simulateQ_pure,
      ...
      erw [map_pure] at h_output_mem_V_run_support
      erw [simulateQ_pure] at h_output_mem_V_run_support
      ...
      · -- masterKStateProp with oStmtOut/witOut → sumcheckRoundRelationProp with
        ...
        have h_cons : sumcheckConsistencyProp (𝓑 := 𝓑) stmtOut_computed.sumcheck_target witOut.H
          := h_relOut.1
        have h_wit_struct :
            witOut.H = Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ')
              (t := witOut.t')
        ...
        have h_h_goal_eq :
            Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ') (t := witOut.t')
        ...
        have h_wit_H_eq_goal :
          ...
          calc
            witOut.H
                = Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ')
                    (t := witOut.t')
                    (m := (RingSwitching_SumcheckMultParam κ L K β ℓ ℓ' h_l).multpoly
                      stmtOut_computed.ctx)
                    (i := 0) (challenges := stmtOut_computed.challenges) := h_wit_struct
            _ = Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ') (t := witOut.t')
                  (m := (RingSwitching_SumcheckMultParam κ L K β ℓ ℓ' h_l).multpoly ctx)
                  (i := 0) (challenges := Fin.elim0) := by
    ...
    · simp only [Fin.isValue, h_V_check, ↓reduceIte, OptionT.run_failure, simulateQ_pure,
      ...
      rw [show ((pure none : StateT σ ProbComp (Option (Statement
        (RingSwitchingBaseContext κ L K ℓ) 0))) s) = pure (none, s) from rfl] at hab
...
theorem batchingReduction_perfectCompleteness (hInit : NeverFail init) :
  ...
  -- The VCVio bump made the specific `pSpecBatching` challenge oracle instances lose the
  -- resolution race to a generic (dead-ending) instance; pin them via explicit `@`-application.
  rw [@OracleReduction.unroll_2_message_reduction_perfectCompleteness _ _ []ₒ _ _
    _ _ _ _ _ _ _ _ _ (pSpecBatching κ L K) _
    (Binius.RingSwitching.instFintypePSpecBatchingChallenge κ L K)
    (Binius.RingSwitching.instInhabitedPSpecBatchingChallenge κ L K) _
    _ _ _ init impl hInit (by rfl) (by rfl)
  ...
  -- Pin the `HasEvalSPMF` instance explicitly (the generic search dead-ends after the VCVio bump).
  ...
  · -- Peel off monadic layers to reach the core verifier logic
    ...
    erw [OptionT.probFailure_liftComp_of_OracleComp_Option
      (superSpec := []ₒ + [(pSpecBatching κ L K).Challenge]ₒ)]
    conv_lhs =>
      enter [1]
    ...
    have h_inputState_eq : inputState = (batchingProverComputeMsg κ L K ℓ ℓ' h_l stmtIn witIn,
        stmtIn, oStmtIn, witIn, batchingProverComputeMsg κ L K ℓ ℓ' h_l stmtIn witIn) :=
      hInputState_mem_support
    have h_inputState1 : inputState.1 = batchingProverComputeMsg κ L K ℓ ℓ' h_l stmtIn witIn := by
      rw [h_inputState_eq]
    have h_V_check_is_true : V_check := by
      ...
      show FullTranscript.mk2 (OracleInterface.toOC.impl ()
          (batchingProverComputeMsg κ L K ℓ ℓ' h_l stmtIn witIn)) r_i'
        = (batchingStepLogic κ L K β ℓ ℓ' h_l aOStmtIn).honestProverTranscript stmtIn witIn oStmtIn _
      rfl
  ...
  · -- GOAL 2: CORRECTNESS - Prove all outputs in support satisfy the relation
    ...
    simp only
    -- Pin the concrete oracle-spec instances (generic search dead-ends after the VCVio bump).
    haveI hSPMF : HasEvalSPMF (OracleComp (([]ₒ : OracleSpec PEmpty) +
      [(pSpecBatching κ L K).Challenge]ₒ)) := instHasEvalSPMFBatchingOracleComp κ L K
    haveI hSet : HasEvalSet (OracleComp (([]ₒ : OracleSpec PEmpty) +
      [(pSpecBatching κ L K).Challenge]ₒ)) := inferInstance
    haveI hFintypeSub : ([]ₒ : OracleSpec PEmpty).Fintype := inferInstance
    haveI hInhabSub : ([]ₒ : OracleSpec PEmpty).Inhabited := inferInstance
    haveI hFintypeChal : [(pSpecBatching κ L K).Challenge]ₒ.Fintype := inferInstance
    haveI hInhabChal : [(pSpecBatching κ L K).Challenge]ₒ.Inhabited := inferInstance
...
lemma batching_pack_unpack_id (t' : Sumcheck.Structured.MultilinearPoly L ℓ') :
...
lemma batching_compute_eq_from_hafter
  ...
  have h_msg :
      Binius.BinaryBasefold.sumcheckConsistencyProp (𝓑 := 𝓑)
        (compute_s0 κ L K β msg0 y)
        (Binius.BinaryBasefold.projectToMidSumcheckPoly (L := L) (ℓ := ℓ') (t := witMid.t')
          (m := (RingSwitching_SumcheckMultParam κ L K β ℓ ℓ' h_l).multpoly
  ...
  have h_bar :
    ...
    exact batching_target_consistency (κ := κ) (L := L) (K := K) (β := β) (ℓ := ℓ)
      ...
      (ctx := { t_eval_point := stmtOStmtIn.1.t_eval_point,
                ...
                r_batching := y })
      rfl
  unfold Binius.BinaryBasefold.sumcheckConsistencyProp at h_msg h_bar
...
theorem batchingOracleVerifier_rbrKnowledgeSoundness :
  ...
  rw [@OracleReduction.probEvent_soundness_goal_unroll_log'
    ...
    (Statement (RingSwitchingBaseContext κ L K ℓ) 0 × ((i : aOStmtIn.ιₛᵢ) → aOStmtIn.OStmtIn i))
    (SumcheckWitness L ℓ' 0) 2 (pSpecBatching κ L K) _ σ _ _
    (instFintypeBatchingSumSpec κ L K) impl prover j stmtOStmtIn witIn initState P _ _]
  ...
  · -- Bound the conditional probability for each transcript
    ...
    let q : OracleQuery [(pSpecBatching (κ := κ) (L := L) (K := K)).Challenge]ₒ
        ((pSpecBatching (κ := κ) (L := L) (K := K)).Challenge ⟨1, by rfl⟩)
      := query (spec := [(pSpecBatching (κ := κ) (L := L) (K := K)).Challenge]ₒ) ⟨⟨1, by rfl⟩, ()⟩
    erw [OracleReduction.probEvent_StateT_run_ignore_state
      (comp := simulateQ (impl.addLift challengeQueryImpl)
        (liftM ((pSpecBatching κ L K).getChallenge ⟨1, rfl⟩)))
      (s := x.2)
