...
| `C6.2` | Construction 6.2 | present-but-incomplete | `ToyProblem.Spec.pSpec`, `Statement`, `OracleStatement`, `Witness`, `accepts`, `inputRelation`, `outputRelation`, `accepts_of_inputRelation` in [Spec/General.lean](../../../ArkLib/ProofSystem/ToyProblem/Spec/General.lean) | same | Three-round `ProtocolSpec` (γ / g / spot-checks) with `OracleInterface` instances. IOR-shaped `inputRelation` / `outputRelation` plus the §6.1 `accepts` decision predicate. Honest prover and `OracleReduction` object are deferred (point-form `accepts_of_inputRelation` covers the completeness obligation). |
